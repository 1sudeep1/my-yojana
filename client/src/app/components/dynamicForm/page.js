        ...
        <form onSubmit={formik.handleSubmit} >
            {props.formFields.map((item) => {
                switch (item.type){
                   ...
                   default:
                    return <Input type={item.type} id={item.label} name={item.name} label={item.label} onBlur={(e) => { props.generateKey(formik.values['projectName'], e) }} onChange={formik.handleChange} value={formik.values[item.label]} placeholder={`Enter ${item.label}`} labelPlacement='outside' />
                        break;
                }
